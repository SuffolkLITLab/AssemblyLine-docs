...
The `<username env var>` variable should contain the username of a [dedicated account](security.mdx#test-accounts) on your docassemble server. The `<password env var>` variable should contain the password of that account.
...
Use the "Story Table" Step to [set values of](#values) multiple fields on multiple pages. It is the Step many of our authors work with the most. In our opinion, it is the most flexible way to fill in fields in your forms, unlike the linear ["I set" Step](#set). This makes a test easier to maintain.
...
- `<target question id>` is the `id`[^id] of the `question` block where you want ALKiln to stop using this table.
- `<variable name>` is the name of an interview variable for a form field. Each table row needs one of these in the first column.
- `<value to set>` is the value to set for this row's variable name.

The table **must** include all the variables and values necessary to get a user from the current page to the page with the given question id. The table **must** have two columns with the headings `var` and `value`.
...
:::warning .there_is_another
Avoid using `.there_is_another` in your Story Table. Read about [using `.target_number` instead](#there-is-another).
:::

**Things to note**

You can use the [Story Table generator](#alkiln_story) to generate a first draft of a full test file.

<NeedProxyVars/>

<!--
:::tip Setting checkboxes
...
1. On pages with `show if` fields, it waits to see if a new field appears
1. If one does appear, it looks through each `<variable name>` again
1. It sets any value it can and repeats this process
1. When no new fields get revealed, ALKiln tries to continue to the next page
...
[^id]: You can find the question `id` in your interview YAML in the `question` block of that page. It is a [good idea for every `question` block in your interview to have a unique id](#id-tip). It can be useful for other blocks too.
...
- How to [inspect and understand the HTML of a web page](https://www.dreamhost.com/blog/how-to-inspect-a-website/)
...
If you do need to use a `noyes` field, you will have to think differently about the value here. Set the variable to `True` if you want the box to be checked. Set it to `False` if you want the box to be unchecked.

Example of making sure a `noyes` checkbox field gets checked in a linear Step:
...
To choose ["none of the above"](https://docassemble.org/docs/fields.html#none%20of%20the%20above) use the word `'None'` in your variable name:
...
The `<page text>` must be inside [double quotation marks](https://www.compart.com/en/unicode/U+0022) and **must never** itself contain double quotation marks inside it. Since docassemble interview text usually has special quotation marks, that is usually easy. See the [troubleshooting section on missing phrases](troubleshooting.mdx#missing-phrase) that talks about special characters.
...
<!-- ### `And I take a screenshot ?(?:named "([^"]+)")?` {#pics} -->
...
- [`Scenario Outline:`](https://cucumber.io/docs/gherkin/reference/#scenario-outline) - Like a `Scenario`, but it can contain `Examples`.
...
The <GTOYS/> tests are made of three stages: the setup, the robot, and the take down.
...
The <KittyLitter/> tests are made of three stages: the setup, the robot, and the take down.
...
The <AutoDIY/> tests show a waiting screen with how long the tests have been running, though we can add a yellow dot if enough users request it.
...
If you used a [story table](#story-table) Step, a Scenario might look more like this:
...
Add a [unique id](https://docassemble.org/docs/modifiers.html#id) to each `question` block of your interview. Those help make [some kinds of test output](#report) more clear.
...
### ALKiln has a lot of open issues {#open-issues}
