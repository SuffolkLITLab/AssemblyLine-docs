...
The mistake may have happened on a page much farther back than you expect with a field ALKiln [skipped](writing_tests.mdx#unused_rows).
...
If you look in a [test suite report](writing_tests.mdx#report) and see that ALKiln [skipped setting a variable](writing_tests.mdx#unused_rows), your test code might be [missing a field or value](#missing) that it needs in order to fill out that field.
